...
class VideoROIExtractor:
    ...
    def detect_landmark(self, frame: np.ndarray) -> np.ndarray:
        ...
        ## 새 크기 계산
        new_w, new_h = int(original_frame_width * scale), int(original_frame_height * scale)
...
'''
        ...
        if original_frame_width > original_frame_height: #가로 영상
            scale = self.resized_frame_width / original_frame_width  # 너비 기준
            new_w, new_h = self.resized_frame_width, int(original_frame_height * scale)
        else:
            scale = self.resized_frame_height / original_frame_height  # 높이 기준
            new_w, new_h = int(original_frame_width * scale), self.resized_frame_height
        ...
        rect, score, _ = self.detector.run(resized_frame, upsample_num_times=2, adjust_threshold=0.2)

        #print('검출된 바운딩 박스', rect)
        #breakpoint()
        ...
        ## 추출한 랜드마크 좌표 값을 원본 크기로 복원
        for i in range(68):
            x_resized = shape.part(i).x
            y_resized = shape.part(i).y
            x_original = int(x_resized / scale)  # 축소된 좌표를 원래 크기로 변환
            y_original = int(y_resized / scale)  # 축소된 좌표를 원래 크기로 변환
    ...
    def save_frames(self, rois: np.ndarray):
        ...
        output_dir = os.path.join('/cropped_mouth_real', f"{video_name}")
