...
if __name__ == "__main__":
    ...
    for video_path, true_label in tqdm(zip(video_paths, true_labels), desc='training'):
        try:
            ...
            R_means_array, L_means_array, M_means_array, local_fps = landmarker.detect_with_calculate()
            
            # Compute PPG signals
            G_R, G_L, G_M = [PPG_G(d, local_fps).compute_signal() for d in [R_means_array, L_means_array, M_means_array]]
            C_R, C_L, C_M = [PPG_C(d, local_fps).compute_signal() for d in [R_means_array, L_means_array, M_means_array]]
            ...
            for ppg in combined_segments:
                fe = FeatureExtractor(config['fps_stanard'], *ppg)
