...
def get_inf_dataloader(info_dataset_config):
    inf_datasets = []
    _dataset_type = None
    #breakpoint()
    for dname, dargs in info_dataset_config.items():
        transformation_cfg = get_default_transformation_cfg(mode='test')
        dataset_type = dargs['dataset_type']
        if _dataset_type is None:
            _dataset_type = dataset_type
        ...
        dargs['transform_cfg'] = transformation_cfg
        #breakpoint()
        inf_datasets.append(__DATASET[dataset_type](**dargs))

    inf_datasets = ConcatDataset(inf_datasets)
    inf_params = {'batch_size': 1,
            'shuffle': False,
            'num_workers': 1,
            'drop_last' : False,
            'collate_fn': __COLLATE_FN[dataset_type],
    ...
    return DataLoader(inf_datasets, **inf_params)
        
'''
{'sdfsdf' : {'data_root': '/home/dongryeol/MM-Det/test_opensora/videocrafter1', 'dataset_type': 'VideoFolderDatasetForReconsWithFn', 'mode': 'test', 'selected_cls_labels': [('0_real', 0)], 'sample_method': 'entire'}'}
'''