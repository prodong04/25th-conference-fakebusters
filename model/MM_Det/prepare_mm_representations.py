...
def LMM_inference(output_dir, output_fn, image_dir, config):
    img_path = os.listdir(image_dir)
    logging.info(f'paths found : {len(img_path)}')

    config['lmm_ckpt'] = 'sparklexfantasy/llava-1.5-7b-rfrd'
    config['load_4bit'] = False
    logging.info(f'LMM - Model Loading...')
    ...
    with torch.inference_mode():
        for img in img_path:
            ...
            torch.save(mm_representations, os.path.join(output_dir, img, output_fn))
...
if __name__ == '__main__':
    ...
    config['lmm_ckpt'] = 'sparklexfantasy/llava-1.5-7b-rfrd'
    config['load_4bit'] = False
    logging.info(f'paths found : {len}')
    ...
    with torch.inference_mode():
        for cls_idx, sub_cls in enumerate(cls_folder, 1):
            ...
            for label in labels:
                ...
                dataset_meta = get_dataset_meta(os.path.join(input_data_root, sub_cls, label))
                mm_representations = {}
                for data_id, count in tqdm(dataset_meta.items()):
                    ...
                    for index in sampled_index:
                        img = Image.open(os.path.join(input_data_root, sub_cls, label, f'{data_id}_{index}.jpg')).convert('RGB')
                        visual_features, mm_features = model(img)
                        mm_layer_features = {}
                        for idx, layer in enumerate(model.selected_layers):
                            mm_layer_features[str(layer)] = mm_features[idx].cpu()
