...
__all__ = ['VisionTransformer', 'WindowVisionTransformerWithIAFA']  # model_registry will add each entrypoint fn to this
...
class IAFABlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        B = x.size(0)
        fc_token = x[:, :self.window_size, :]
        sequence_token = x[:, self.window_size: , :]
        sequence_token = sequence_token + self.drop_path1(self.ls1(self.attn(self.norm1(sequence_token))))
        sequence_token = sequence_token + self.drop_path2(self.ls2(self.mlp(self.norm2(sequence_token))))
        sequence_token = rearrange(sequence_token, 'b (s w) m -> (b w) s m', b=B, w=self.window_size)
        fc_token = rearrange(fc_token, 'b w m -> (b w) m').unsqueeze(1)
        x = torch.cat([fc_token, sequence_token], dim=1)
        x = x + self.temporal_attn(self.temporal_norm1(x))
        x = x + self.temporal_mlp(self.temporal_norm2(x))
        sequence_token = x[:, 1:, :]
        sequence_token = rearrange(sequence_token, '(b w) s m -> b (s w) m', b=B, w=self.window_size)
        fc_token = x[:, 0, :]
        fc_token = rearrange(fc_token, '(b w) m -> b w m', b=B)
        x = torch.cat([fc_token, sequence_token], dim=1)
...
class WindowVisionTransformerWithIAFA(nn.Module):
    """ Window Vision Transformer

    The implementations of ViT for video patches on fixed window size.
    """
    dynamic_img_size: Final[bool]
    ...
    def __init__(
            ...
            norm_layer: Optional[LayerType] = None,
            act_layer: Optional[LayerType] = None,
            block_fn: Type[nn.Module] = IAFABlock,
            mlp_layer: Type[nn.Module] = Mlp,
            **kwargs
    ) -> None:
        """
        ...
        num_patches = self.patch_embed.num_patches     # here, num_patches equals the patch number of all frames
        self.window_size = self.patch_embed.window_size    # the window size from patch embedding

        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if class_token else None
        self.fc_token = nn.Parameter(torch.zeros(1, self.window_size, embed_dim))
        self.reg_token = nn.Parameter(torch.zeros(1, reg_tokens, embed_dim)) if reg_tokens else None
        pos_embed_len = 197
        embed_len = num_patches // self.window_size if no_embed_class else num_patches // self.window_size + self.num_prefix_tokens
        self.pos_embed = nn.Parameter(torch.randn(1, pos_embed_len, embed_dim) * .02)    # divide the pos_embed to the original dimension for pretrained weights
        self.window_pos_embed = nn.Parameter(torch.randn(1, embed_len, embed_dim) * .02)    # the real pos_embed
        self.temporal_pos_embed = nn.Parameter(torch.randn(1, self.window_size, embed_dim) * .02)
        self.pos_drop = nn.Dropout(p=pos_drop_rate)
        self.time_embed = nn.Parameter(torch.randn(1, self.window_size, embed_dim)* .02)
    ...
    def init_weights(self, mode: Literal['jax', 'jax_nlhb', 'moco', ''] = '') -> None:
        ...
        if self.cls_token is not None:
            nn.init.normal_(self.cls_token, std=1e-6)
        nn.init.normal_(self.fc_token, std=1e-6)
        named_apply(get_init_weights_vit(mode, head_bias), self)
    ...
    # sort out sizes, assume square if old size not provided
        ...
        num_new_tokens = new_size + num_prefix_tokens
        if num_new_tokens == num_pos_tokens:
            return posemb

        if old_size is None:
            old_size = num_pos_tokens - num_prefix_tokens
        ...
        posemb = posemb.reshape(1, old_size, -1).permute(0, 2, 1)
        posemb = F.interpolate(posemb, size=new_size)
        posemb = posemb.permute(0, 2, 1).reshape(1, -1, embed_dim)
    ...
    def _pos_embed(self, x: torch.Tensor) -> torch.Tensor:
        if self.dynamic_img_size:
            B, H, W, C = x.shape
            pos_embed = resample_abs_pos_embed(
                self.window_pos_embed,
                (H, W),
                num_prefix_tokens=0 if self.no_embed_class else self.num_prefix_tokens,
            )
            x = x.view(B, -1, C)
        else:
            pos_embed = self.window_pos_embed
        ...
        if self.no_embed_class:
            ...
            sequence_pos_embed = pos_embed[0,1:,:].unsqueeze(0)
            temporal_sequence_pos_embed = []
            for i in range(self.window_size):
                temporal_sequence_pos_embed.append(self.temporal_pos_embed[:, i, :].unsqueeze(1))
                temporal_sequence_pos_embed.append(sequence_pos_embed)
            temporal_sequence_pos_embed = torch.cat(temporal_sequence_pos_embed, dim=1)
            
            if to_cat:
                temporal_sequence_to_cat = []
                frame_patches = x.size(1) // self.window_size
                for i in range(self.window_size):
                    temporal_sequence_to_cat.append(self.fc_token[:, i, :].unsqueeze(1).expand(x.shape[0], -1, -1))
                    temporal_sequence_to_cat.append(x[:, i * frame_patches: (i + 1) * frame_patches, :])
                x = torch.cat(temporal_sequence_to_cat, dim=1)
            # x = x + pos_embed
            x = x + temporal_sequence_pos_embed

        return self.pos_drop(x)
    
    def _time_embed(self, x: torch.Tensor, window_size: int) -> torch.Tensor:
        sequence_tokens = x
        sequence_time_embed = self.time_embed
        if window_size != self.window_size:
            sequence_time_embed = rearrange(sequence_time_embed, 'b w d -> b d w')
            sequence_time_embed = F.interpolate(sequence_time_embed, size=(window_size), mode='nearest')
            sequence_time_embed = rearrange(sequence_time_embed, 'b d w -> b w d')
        sequence_time_embed = torch.repeat_interleave(sequence_time_embed, sequence_tokens.size(1) // window_size, dim=1)
        x = sequence_tokens + sequence_time_embed
        x = self.time_drop(x)
        x = rearrange(x, 'b (w t) m -> (b w) t m', w=window_size)
        fc_token = x[:, 0, :].unsqueeze(1)
        sequence_token = x[:, 1:, :]
        fc_token = rearrange(fc_token, '(b w) t m -> b (w t) m', w=window_size)
        sequence_token = rearrange(sequence_token, '(b w) t m -> b (w t) m', w=window_size)
        x = torch.cat([fc_token, sequence_token], dim=1)
        return x
    ...
    def forward_features(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        x = self._pos_embed(x)
        window_size = x.size(1) // self.window_pos_embed.size(1)
        x = self._time_embed(x, window_size=window_size)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.blocks, x)
    ...
    def forward_head(self, x: torch.Tensor, pre_logits: bool = False) -> torch.Tensor:
        if self.attn_pool is not None:
            ...
            x = x[:, :self.window_size]  # class token
        x = self.fc_norm(x)
        x = self.head_drop(x)
        return x if pre_logits else self.head(x)
...
def _create_window_vision_transformer_with_iafa(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    return build_model_with_cfg(
        WindowVisionTransformerWithIAFA,
        variant,
        pretrained,
        pretrained_filter_fn=_filter_fn,
        pretrained_strict=strict,
