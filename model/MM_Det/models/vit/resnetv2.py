...
class ResNetV2(nn.Module):
    ...
    def __init__(
        ...
        from .frequency_enhancer import GaussianBlurProcessor
        self.blur_kernel_sizes = [3, 5, 7]
        self.blur_processor = GaussianBlurProcessor(kernel_sizes=self.blur_kernel_sizes)
        self.freq_stem = create_resnetv2_stem(
            in_chans,
            stem_chs,
            stem_type,
            preact,
        ...
        # stems fusion in resnet stage 0
        self.stems_transition_conv_1x1 = nn.Sequential(
        ...
        # multi stems initialization
        for m in self.stems_transition_conv_1x1.modules():
        ...
        for stage_idx, (d, c, bdpr) in enumerate(zip(layers, channels, block_dprs)):
            ...
            if stage_idx == 0:         
                # in stage 0, frequency stage is indenpendent from resnet stage.
                self.freq_stage = ResNetStage(
                    len(self.blur_kernel_sizes) * prev_chs,
                    out_chs,
                    stride=stride,
                    dilation=dilation,
                    depth=d,
                ...
                # frequency stage initialization
                for m in self.freq_stage.modules():
                    if isinstance(m, nn.Conv2d):
                        nn.init.kaiming_normal_(m.weight)
                    elif isinstance(m, nn.BatchNorm2d):
                        nn.init.constant_(m.weight, 1)
    ...
    def forward_features(self, x_input):
        x = self.stem(x_input)
        x_freq = self.freq_stem(x_input)
        x_blurs = self.blur_processor(x_freq)
        x_freq_resi = x_freq.repeat(1, x_blurs.shape[1] // x_freq.shape[1], 1, 1) - x_blurs
        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.stages, x, flatten=True)
        else:
            # x = self.stages(x)
            for idx, ResNet_block in enumerate(self.stages):
                x = ResNet_block(x)
                if idx == 0:
                    x_freq = self.freq_stage(x_freq_resi)
                    x = torch.cat([x, x_freq], dim=1).to(x.device)
                    x = self.stems_transition_conv_1x1(x)
        x = self.norm(x)
        return x
...
class ResNetV2TwoBranchRecons(nn.Module):
    """Implementation of Pre-activation (v2) ResNet mode.
        Apply multi stem branches.
    """
    ...
    def forward_features(self, x_input):
        x_rgb, x_resi = x_input
        x = self.stem(x_rgb)
        x_freq = self.freq_stem(x_resi)
        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.stages, x, flatten=True)
        else:
            # x = self.stages(x)
